...
| Work Zone app naming | Content Manager shows a local `FreshChain Rescue Cockpit` content item assigned to the `FreshChain Command` group, but the runtime home page still paints the old `FreshChain Sense` label. The opened app title and content are correct. | Judges may not immediately know this is the main live-action demo app. | Verbally introduce the stale-labelled tile as the Rescue Cockpit; click it and show the opened app title. | Republish/refresh the Work Zone site page so the home tile metadata catches up with the local content item, or replace the stale runtime tile with the local `FreshChain Rescue Cockpit` item. |
