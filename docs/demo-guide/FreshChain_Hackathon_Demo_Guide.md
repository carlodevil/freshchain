...
### 4:55-5:35 — Open Manage Workflow
![Open Manage Workflow](screenshots/10-operations.png)

**Button/path:** Open Manage Workflow.
**Say:** Show the operational queue as the frontline exception workbench. Point to the newest critical/open rows first, severity/status colouring, store and zone, owner, outcome, and row actions. Explain that this is where store and operations teams assign, acknowledge, resolve, and reopen spoilage interventions outside the scripted rescue cockpit.
**Concrete outcome:** The solution maps from executive incident to frontline action.

### 5:35-6:10 — Open View Risk
![Open View Risk](screenshots/11-intelligence.png)

**Button/path:** Open View Risk.
...
### 6:10-6:40 — Open View Financials / Monitor / Review Ingestion Errors
![Open View Financials / Monitor / Review Ingestion Errors](screenshots/12-admin-prove.png)

**Button/path:** Open View Financials / Monitor / Review Ingestion Errors.
**Say:** Use View Financials for the money trail: scenario ID, actioned/potential status, stock value at risk, protected revenue, movement reference, and calculation summary. Use Monitor for the operational health trail: active alerts by zone and severity. Use Review Ingestion Errors only to show exception handling; if it is empty, say that the current feed has no quarantined payloads.
...
|---|---|---|---|---|
| Work Zone first-row KPIs | Resolved on 2026-06-18 at 05:28 UTC. The `FreshChain Command` group now starts with live dynamic tiles whose titles describe the opened app function: `View Command Center`, `View Risk`, `Manage Workflow`, and `View Financials`. Each tile opens a different Fiori app. | No first-viewport KPI gap remains for the panel flow. Values are still live and time-sensitive. | Read the current values from the Work Zone tiles during rehearsal and the panel demo. If any tile fails, state the live defect rather than using the screenshot as proof. | Keep these tiles first in the command group and recapture the screenshot after the final rehearsal run if values materially change. |
| Work Zone app naming | Resolved for the visible demo path on 2026-06-18 at 04:58 UTC. The HTML5 Apps provider was synced after the dist redeploy, the CAP dynamic tile payload was corrected, and the local Rescue Cockpit CDM visualization text now says `Run live rescue flow` / `Live demo`. Runtime home no longer shows the old KPI-only copy. The technical compatibility hash still uses `FreshChainSense-display`. | No visible naming issue remains for the panel flow. The technical hash is still legacy, but the label and opened app title are correct. | Open the visible `FreshChain Rescue Cockpit` tile from Work Zone. Do not mention `Sense` unless asked about the URL/hash history. | Later, replace the compatibility hash with `FreshChainRescueCockpit-display` once Work Zone target resolution accepts the clean semantic object for this site. |
...
| Supporting app depth | Improved on 2026-06-17. Act, Predict, Prove, and Monitor now sort live evidence newest-first and foreground live-derived criticality, action owner/outcome, financial proof, and alert status. They remain Fiori evidence surfaces rather than separate cockpit experiences. | Lower demo risk: secondary apps now support the story instead of looking like generic admin tables. | Keep these screens short: Act = frontline work queue, Predict = AI decision log, Prove = financial/audit proof, Monitor = alert health, Ingestion Errors = quarantine proof. | After redeploy, recapture the Act/Predict/Prove/Monitor screenshots from the live tenant if their first visible rows change materially. |
...
## Capture Notes
- Work Zone screenshot was captured from: https://afsug-hackathon.launchpad.cfapps.eu20.hana.ondemand.com/site?siteId=97d11aec-866b-4a20-8f25-695b8927576e#Shell-home after syncing the HTML5 Apps channel and updating the local group CDM to one visible tile per app. Treat any displayed KPI as presentation proof only after the current value is verified against live persisted CAP/HANA data.
...
- Work Zone launch paths validated on the live tenant after the service fix: View Command Center, FreshChain Rescue Cockpit, Manage Workflow, View Risk, View Financials, FreshChain Monitor, and Review Ingestion Errors. Browser capture reported no bad or failed FreshChain/OData requests; remaining console noise was SAP shell deprecations/plugin warnings.
- Current live validation after the HTML5 dist redeploy, provider sync, CAP dynamic tile payload fix, and local CDM text fix: the Work Zone home page shows `FreshChain Rescue Cockpit`, `Run live rescue flow`, and `Live demo`, and it no longer shows the old KPI-only Rescue Cockpit copy. The technical compatibility hash `#FreshChainSense-display` still opens the app with the corrected title; `#FreshChainRescueCockpit-display` remains a future cleanup once target resolution accepts the clean semantic object.
- Current live validation after the KPI group update: the Work Zone first row shows `View Command Center`, `View Risk`, `Manage Workflow`, and `View Financials` as distinct dynamic KPI launchers before the remaining app launch tiles. Browser click validation confirmed each opens a different Fiori app and reported no failed FreshChain/OData requests.
- Current live validation after the supporting-app metadata update: Manage Workflow and Monitor show severity/status plus recommendation in the first viewport, View Risk shows newest AI Core risk decisions first, and View Financials shows actual protected revenue, potential protected revenue, stock at risk, and expected loss in the first viewport. The valid technical hash for Monitor is `#FreshChainMonitoring-display`.
