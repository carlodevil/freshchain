...
**Button/path:** Open FreshChain Rescue Cockpit. If the live home page still shows the legacy `FreshChain Sense` tile, use that tile as the compatibility entry point and then press Start incident.
...
- If a tile label still says `FreshChain Sense`, state plainly that it is a legacy Work Zone visualization that still launches the Rescue Cockpit. After the latest HTML5 content sync, the clean `FreshChainRescueCockpit-display` intent should be used for new content assignments.
...
|---|---|---|---|---|
| Work Zone secondary KPIs | The manifest contains dynamic KPI inbounds for protected revenue, stock at risk, rescue proof, and waste avoided, but Content Manager currently exposes the imported Control Tower visualization as one dynamic tile using `DynamicTileKpis('protectedRevenue')`. The runtime first viewport shows that live protected-revenue tile plus mostly static app launch tiles. | The first impression does not yet tell the full operational story before opening apps. | Start with the live protected-revenue tile, then open Control Tower for stock at risk, rescue proof, BTP readiness, and task evidence. If a tile does not resolve, show it as a defect. | In Work Zone page/content management, add first-viewport live KPI cards/tiles for stock at risk, rescue proof, and waste avoided only when each resolves from live CAP/HANA data. |
| Work Zone app naming | Live CDM inspection shows the local `FreshChain Rescue Cockpit` item has a correct item title but its visualization/default target still contains `FreshChain Sense` and uses `FreshChainSense-display`. The repo manifest now also exposes `FreshChainRescueCockpit-display` while keeping `FreshChainSense-display` as a legacy alias; the full HTML5 app set was redeployed from rebuilt `dist` folders at 22:02 UTC. | Judges may not immediately know this is the main live-action demo app, and direct navigation to the new clean intent will not resolve until the HTML5 Apps provider/site content is synced and the page assignment uses the new visualization. | Use the legacy tile as a compatibility entry point if needed, then show the opened Rescue Cockpit title and live action flow. | Sync the HTML5 Apps provider, then replace the stale local visualization or page assignment with the clean `FreshChainRescueCockpit-display` content item. |
...
- Work Zone launch paths validated on the live tenant after the service fix: FreshChain Rescue Command Center, FreshChain Rescue Cockpit through the legacy `FreshChainSense-display` compatibility tile, FreshChain Act, FreshChain Predict, FreshChain Prove, FreshChain Monitor, and FreshChain Ingestion Errors. Browser capture reported no bad or failed FreshChain/OData requests; remaining console noise was SAP shell deprecations/plugin warnings.
- Current live validation after the HTML5 dist redeploy: `#FreshChainRescueCockpit-display` still does not resolve until Work Zone provider/site content is synced, while `#FreshChainSense-display` still opens the app. Treat that as a Work Zone content refresh task, not as permission to rename the app back to Sense.
